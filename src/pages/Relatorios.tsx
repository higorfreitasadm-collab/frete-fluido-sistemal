...
export default function Relatorios() {
  ...
  const remetenteRanking = Object.entries(nfs.reduce((acc, nf) => {
    acc[nf.remetente] = (acc[nf.remetente] || 0) + nf.frete;
    return acc;
  }, {} as Record<string, number>)).map(([name, valor]) => ({ name, valor })).sort((a, b) => b.valor - a.valor);

  const destinatarioRanking = Object.entries(nfs.reduce((acc, nf) => {
    acc[nf.destinatario] = (acc[nf.destinatario] || 0) + nf.frete;
    return acc;
  }, {} as Record<string, number>)).map(([name, valor]) => ({ name, valor })).sort((a, b) => b.valor - a.valor);

  const responsavelRanking = Object.entries(nfs.reduce((acc, nf) => {
    const nome = mockUsers.find(u => u.id === nf.responsavel_id)?.nome ?? 'Desconhecido';
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Análises e indicadores das notas fiscais</p>
          </div>
